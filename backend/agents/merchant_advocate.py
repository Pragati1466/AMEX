...
def run_merchant_advocate(case_text: str) -> Argument:
    return invoke_with_retry(
        invoke_fn=lambda: structured_llm.invoke([
            SystemMessage(content=SYSTEM_PROMPT),
            HumanMessage(content=f"Case evidence:\n\n{case_text}\n\nBuild the merchant's defense.")
        ]),
        fallback=fallback_argument("merchant")
    )
...
if __name__ == "__main__":
    ...
    """
    result = run_merchant_advocate(mock_case)

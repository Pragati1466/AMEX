    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 p-1">
      ...
      <div className="space-y-5 lg:col-span-2">
        ...
        <Section title="Dispute Information" icon={FileText}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <InfoRow label="Amount" value={header.amount ? `${header.currency || 'USD'} ${Number(header.amount).toLocaleString()}` : null} />
        ...
        <Section title="Customer" icon={User}>
          ...
          <InfoRow label="Verified" value={header.customer_verified != null ? (header.customer_verified ? 'Yes' : 'No') : null} />
        </Section>
      ...
      <div className="space-y-5">
        ...
        <Section title="Fairness Score" icon={CheckCircle}>
          {fairness.available === false ? (
            <p className="text-xs text-gray-400">{fairness.message || 'Not yet available'}</p>
          ) : (
            <>
              <div className={`text-3xl font-bold ${scoreColor(fairness.fairness_score)} mb-2`}>
              ...
              {fairness.score_explanation && (
                <p className="text-xs text-gray-500 mt-2">{fairness.score_explanation}</p>
              )}
              {fairness.confidence != null && (
                <p className="text-xs text-gray-400 mt-1">AI Confidence: {formatPct(fairness.confidence)}</p>
              )}
            </>
          )}
        </Section>
        ...
        <Section title="AI Recommendation" icon={AlertTriangle}>
          {rec.available === false ? (
            <p className="text-xs text-gray-400">{rec.message || 'Not yet available'}</p>
          ) : (
            <>
              ...
              />
              {rec.rationale && (
                <p className="text-xs text-gray-600 mt-3 leading-relaxed">{rec.rationale}</p>
              )}
        ...
        <Section title="Evidence Completeness" icon={Clock}>
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium">{completeness.completeness_pct != null ? `${completeness.completeness_pct.toFixed(1)}%` : '—'}</span>
            </div>
            <ScoreBar score={completeness.completeness_pct} />
            <div className="flex justify-between text-xs">
              <span className="text-gray-500">Total Evidence</span>
              <span className="font-medium">{completeness.total_evidence ?? 0}</span>
            </div>
            {completeness.missing_evidence?.length > 0 && (
              <div>
                <p className="text-xs text-red-600 font-medium mt-2">Missing:</p>
            ...
            {completeness.critical_gaps?.length > 0 && (
              <div>
                <p className="text-xs text-orange-600 font-medium mt-2">Critical Gaps:</p>
                <ul className="text-xs text-gray-600 mt-1 space-y-0.5">
        ...
        {decision && (
          <Section title="Final Decision" icon={CheckCircle}>
            <div className="space-y-1">
              <div className="text-sm font-semibold text-green-700">{formatOutcome(decision.outcome)}</div>
              <div className="text-xs text-gray-500">
                <span className="capitalize">{decision.decision_type}</span> · {formatDateTime(decision.created_at)}
              </div>
              {decision.rationale && (
                <p className="text-xs text-gray-600 mt-2 leading-relaxed">{decision.rationale}</p>
              )}
            </div>
          </Section>
        )}
